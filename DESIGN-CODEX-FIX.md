...
6. complete token exchange using the stored PKCE verifier and the correct resource-specific auth context
...
A single listener-only instance can serve callbacks for multiple resources because callback routing is state-based.
...
### Start tenancy 1
```bash
npx --yes --package ~/.mcp-auth/mcp-remote-<package-version>.tgz mcp-remote https://mcp.atlassian.com/v1/mcp --resource https://tenancy1.atlassian.net/ --callback-port 3736 --debug
```

### Start tenancy 2
```bash
npx --yes --package ~/.mcp-auth/mcp-remote-<package-version>.tgz mcp-remote https://mcp.atlassian.com/v1/mcp --resource https://tenancy2.atlassian.net/ --callback-port 3736 --debug
```
